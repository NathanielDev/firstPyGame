...
class Player():
    def __init__(self, x, y, width, height, pBulletW, pBulletH, pBulletV, pBulletC, pBulletM, spriteID, isP1,
                 isLocal):
        self.x = x
        self.y = y
        self.width = width
        ...
        #         pygame.transform.scale(pygame.image.load(spritePath), (width, height)), 180)
    ...
    def shoot(self):  # TODO: center player shots
        Bullet(self.rect.x + 20, self.rect.y + 20, self.pBulletW, self.pBulletH, self.pBulletV, self.pBulletC,
               self.pBulletM, handler=self.pbList)  # braindead moment
